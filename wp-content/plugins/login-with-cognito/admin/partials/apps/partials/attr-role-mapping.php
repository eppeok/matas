...
/**
 * Attribute role mapping tab UI
 */
function mocognito_attribite_role_mapping_ui() {
	$appslist       = get_option( 'mo_oauth_apps_list' );
	$attr_name_list = get_option( 'mo_oauth_attr_name_list' );

	if ( false !== $attr_name_list ) {
		$temp           = array();
		$attr_name_list = mocognito_dropdownattrmapping( '', $attr_name_list, $temp );
	}
	$currentapp     = null;
	$currentappname = null;
...
>
						...
						<script>
						function mocognito_changeFormField(){
						...
						<input type="hidden" id="mo_attr_option" name="mo_attr_option" value="<?php //@phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentBeforeOpen, Squiz.PHP.EmbeddedPhp.ContentAfterOpen, Squiz.WhiteSpace.SuperfluousWhitespace.EndLine -- Ignoring as spaces affecting input value
						...
						if ( get_option( 'mo_attr_option' ) === 'automatic' || get_option( 'mo_attr_option' ) === false ) {
							echo 'style="display:none"';}
						?>
						placeholder="Enter attribute name for Username" type="text" id="mo_oauth_username_attr_input" value="<?php //@phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentBeforeOpen, Squiz.PHP.EmbeddedPhp.ContentAfterOpen, Squiz.WhiteSpace.SuperfluousWhitespace.EndLine -- Ignoring as spaces affecting input value
						...
						<td>
							<a href="#" id="mo_username_attr_change_p" onclick="mocognito_changeFormField()"><?php echo esc_attr( $text_attr ); ?></a>
						</td>
						<?php
					} else {
						?>
						<input class="mo_table_textbox" required="" placeholder="Enter attribute name for Username" type="text" id="mo_oauth_username_attr_input" name="mo_oauth_username_attr" value="<?php //@phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentBeforeOpen, Squiz.PHP.EmbeddedPhp.ContentAfterOpen, Squiz.WhiteSpace.SuperfluousWhitespace.EndLine -- Ignoring as spaces affecting input value
						if ( isset( $currentapp['username_attr'] ) ) {
							echo esc_attr( $currentapp['username_attr'] );
						} elseif ( isset( $currentapp['email_attr'] ) ) {
							echo esc_attr( $currentapp['email_attr'] );}
		...
		echo '<tr>
            ...
            Advanced attribute mapping is available in <a href="admin.php?page=mo_oauth_settings&amp;tab=licensing">Standard</a> version.</b>
            </p>
			</td>
		</tr>
 ...
 *
 * @param NestedPrefix         $nestedprefix nexted JSON struture path.
 *
 * @param ResourceOwnerDetails $resource_owner_details resource owner details.
 *
 * @param Temp                 $temp temp.
 */
function mocognito_dropdownattrmapping( $nestedprefix, $resource_owner_details, $temp ) {
	foreach ( $resource_owner_details as $key => $resource ) {
		if ( is_array( $resource ) ) {
			if ( ! empty( $nestedprefix ) ) {
				$nestedprefix .= '.';
			}
			$temp         = mocognito_dropdownattrmapping( $nestedprefix . $key, $resource, $temp );
